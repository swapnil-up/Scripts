...
import os
import tempfile
from utils import run_ffmpeg, validate_file, get_duration, format_time, sidecar_path, auto_output_path
...
def process_marked_cuts(input_file, output_file, fast=True):
    ...
    if len(markers) % 2 != 0:
        print(f"ERROR: Odd number of markers ({len(markers)}). Each cut needs a start AND end.")
    ...
    try:
        ...
        print(f"  Cut out  : {format_time(cut_duration)} ({cut_duration / duration * 100:.1f}%)")
